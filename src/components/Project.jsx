      ...
      <div className="bg-white/10 backdrop-blur-xl border border-white/10 rounded-xl p-6 md:p-10 shadow-lg w-full max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {PROJECTS.map((project, index) => (
            <div
              key={index}
              className="bg-neutral-800 rounded-lg p-6 shadow-lg flex flex-col h-full"
            >
              <img
                src={project.image}
                alt={project.title}
              ...
              <p className="text-neutral-300 mb-3 text-sm md:text-base flex-grow">
                {project.description}
              </p>
              <div className="flex flex-wrap gap-2 mt-auto">
                {project.technologies.map((tech, i) => (
                  <span
                    key={i}
                    className="bg-fuchsia-500 text-xs md:text-sm px-2 py-1 rounded"
                  >
                    {tech}
                  </span>
                ))}
